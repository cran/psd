...
## @knitr eval=TRUE, echo=TRUE, label="Filtered signal analogy."
set.seed(1234)
N <- 1028
...
## @knitr eval=TRUE, echo=FALSE, fig.width=6, fig.height=3, label=FILTERS
...
## @knitr eval=TRUE, echo=TRUE, label="Synthetic white noise and a DFT."
...
## @knitr eval=TRUE, echo=TRUE, label="Amplitude and phase spectra."
...
## @knitr eval=TRUE, echo=TRUE, label="Nyquist frequencies."
...
## @knitr eval=TRUE, echo=TRUE, label=OPTIM_EXPECT
...
## @knitr eval=TRUE, echo=TRUE, label=MEAN_EXPECT
# compare with the mean and median
c(mSn <- mean(S), median(S))


## @knitr eval=TRUE, echo=FALSE, fig.width=5, fig.height=5, label=QQFIT
par(pty="s", las=1)
...
## @knitr eval=TRUE, echo=FALSE, fig.width=6, fig.height=3.5, label=PSD
...
## @knitr eval=TRUE, echo=TRUE, label="Test normalization."
mSn <- dof
...
## @knitr eval=TRUE, echo=TRUE, label="Apply correct normalization."
fsamp <- 20
fNyq <- fsamp / 2
freqs <- fsamp * nyfreqs 
Snew <- S / fsamp
...
## @knitr eval=TRUE, echo=TRUE, label="DB"
# decibel function
dB <- function(y) 10*log10(y)


## @knitr eval=TRUE, echo=FALSE, fig.width=6, fig.height=3.8, label=PSD2
par(las=1)
...
## @knitr eval=TRUE, echo=TRUE, label="Change the sampling frequency."
fsamp <- 20
...
## @knitr eval=TRUE, echo=FALSE, fig.width=6, fig.height=3.5, label=NORMS
par(las=1)
...
## @knitr eval=TRUE, echo=TRUE, label="Test the normalization again."
test_norm(mean(pgram01$spec), 0.5, xv)
test_norm(mean(pgram20$spec), 10, xv)


## @knitr eval=TRUE, echo=TRUE, label="Double sided PSD from spectrum."
